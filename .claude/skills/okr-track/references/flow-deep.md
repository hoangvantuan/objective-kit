...
Hỏi user có update progress nào trước phân tích (giống light). Nếu có → áp dụng + log + **nhắc output cho actions done** (flow-light step 4) + **archive actions done** (flow-light step 5). Bao gồm sync pull/push (flow-light step 1 + 3b) và re-render Roadmap (flow-light step 5) như light.
...
Chạy Inbox Processing Flow (xem `flow-inbox.md`).
