...
## C2: Gom cụm (gom trước, xử lý sau)
...
Áp primitive tương ứng cho mỗi dòng user GIỮ (xem mục "Mô hình ghi"). Sau đó cập nhật `index.md`: 3 bảng chính (skill / workflow / project) + bảng Lưu trữ. Bài thành `obsolete` chuyển từ bảng chính xuống Lưu trữ. Bài mới (tách / reclassify) thêm dòng vào bảng chính đúng ngăn. Bài thăng scope: cập nhật dòng workflow tại chỗ (cột scope `local`→`shared`, status `active`→`pending`), giữ nguyên vị trí. Áp reorder nếu user giữ.
