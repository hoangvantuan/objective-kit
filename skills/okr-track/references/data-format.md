...
**closure** (từ closure): giống review + thêm section `## Tổng kết period`.
...
...
## Tổng kết period
(chỉ closure: KR cuối vs target, % thời gian, tốc độ hoàn thành, điểm ước lượng từng trượt; tổng kết riêng cho period này)
~~~

> Bài học **tái dùng** (cải tiến skill / tri thức project) KHÔNG ghi vào log closure. Đó là việc của `okr-retro` (ghi `.okr/lessons/`). Closure chỉ gợi ý chạy `okr-retro` (xem `flow-closure.md` Bước 7).
...
> Trace (xem lại history/archive/log) là **mode read-only của `okr-analyze`**, không còn thuộc `okr-track`. Xem `okr-analyze/references/flow-trace.md`.
